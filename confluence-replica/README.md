...
Часто начинается один и тот же цирк:

- агент пытается использовать `atlassian-mcp`
- падает
- клонирует `atlassian-mcp` в `/tmp`
- правит код на лету
- запускает «исправленную» версию
- снова падает на следующем запросе
- объявляет MCP проклятым и уходит в `curl`

`confluence-replica` нужен, чтобы этот жанр геройства просто исчез: никаких полевых ремонтов `atlassian-mcp`, только стабильный локальный контекст для агента.
